...
везде, кроме функций `num`, `str`, `bool`, `echo` и `reply`, о которых речь пойдет ниже. Все остальные функции могут
принимать только идентификаторы.
...
Получают n-ый элемент и удаляют соответственно
...
## Циклы и трансформации

Для понимания этого раздела вам необходимо понимать [собственные функции](#собственные-функции-и-функция-ret).

### Функция `each`

Вы можете пройтись циклом по любой коллекции, используя функцию `each` и любую собственную функцию, при условии, что
собственная функция принимает один элемент:
...
Точно также проходят по коллекции, мутируя и фильтруя соответственно
